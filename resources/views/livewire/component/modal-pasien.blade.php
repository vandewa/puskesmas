                        ...
                        @foreach ($posts as $item)
                            <tr>
                                ...
                                <td>{{ $item->birth_date ?? '' }}
                                    ...
                                    Th</td>
                                <td>
                                    {{ $item->provinsi->region_nm ?? '' }}, {{ $item->kabupaten->region_nm ?? '' }},
                                    {{ $item->kecamatan->region_nm ?? '' }}, {{ $item->kelurahan->region_nm ?? '' }}
                                    {{ $item->address ?? '' }}
                                </td>

                                <td>{{ $item->identity_no ?? '-' }}</td>
                                <td>
                                    <button type="button" wire:click='pilih({{ $item->pasien_cd }})'
                                        class="btn btn-warning btn-flat btn-sm" data-toggle="tooltip"
                                        data-placement="left" title="Pilih Pasien"><i class="fas fa-plus"></i>
                                    </button>
